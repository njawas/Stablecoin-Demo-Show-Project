...
module.exports = async function(deployer, network, accounts) {
	...
	pair_instance_XUS_XUSD = await ERC20.at(pair_addr_XUS_XUSD);
	pair_instance_BAC_XUSD = await ERC20.at(pair_addr_BAC_XUSD);

	let bal2 = await pair_instance_XUSD_WETH.balanceOf.call(owner_addr);
	console.log(`XUSD/WETH LP balance: ${bal2}`);
	let bal4 = await pair_instance_XUS_XUSD.balanceOf.call(owner_addr);
	console.log(`XUS/XUSD LP balance: ${bal4}`);
	let bal5 = await pair_instance_BAC_XUSD.balanceOf.call(owner_addr);
	console.log(`BAC/XUSD LP balance: ${bal5}`);
	let bal6 = await xusdInstance.balanceOf.call(owner_addr);
	console.log(`XUSD balance: ${bal6}`);

	console.log(chalk.yellow.bold('======== Initialize the staking rewards ========'));
	await Promise.all([
		stakingInstance_XUSD_WETH.initializeDefault(),
		stakingInstance_XUS_XUSD.initializeDefault(),
		stakingInstance_BAC_XUSD.initializeDefault(),
		stakingInstance_XUSD.initializeDefault()
	]);
	
	// add to XUS staking pools
	console.log(`=====Adding reward pools to XUS contract=====`);
	await xusInstance.addRewardPool.sendTransaction(stakingInstance_XUSD_WETH.address, new BigNumber("2000000e18"));
	await xusInstance.addRewardPool.sendTransaction(stakingInstance_XUS_XUSD.address, new BigNumber("5000000e18"));
	await xusInstance.addRewardPool.sendTransaction(stakingInstance_BAC_XUSD.address, new BigNumber("2000000e18"));
	await xusInstance.addRewardPool.sendTransaction(stakingInstance_XUSD.address, new BigNumber("1000000e18"));
};
