...
module.exports = async function(deployer, network, accounts) {
	...
	let pool_dai = await Pool_DAI.deployed();
	let pool_weth = await Pool_WETH.deployed();

	console.log(`=====Pausing Collateral Ratio Update=====`);
	// await xusdInstance.toggleCollateralRatio.sendTransaction();

	console.log(`=====Pausing Pool_DAI Mint=====`);
	await pool_dai.toggleMinting.sendTransaction();
	// await pool_dai.toggleRedeeming.sendTransaction();
	// await pool_dai.toggleRecollateralize.sendTransaction();
	// await pool_dai.toggleBuyBack.sendTransaction();

	console.log(`=====Pausing Pool_WETH Mint=====`);
	await pool_weth.toggleMinting.sendTransaction();
	// await pool_weth.toggleRedeeming.sendTransaction();
	// await pool_weth.toggleRecollateralize.sendTransaction();
	// await pool_weth.toggleBuyBack.sendTransaction();
};
